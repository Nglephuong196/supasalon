...
import { type NewProductCategory, productCategories } from "../db/schema";
...
export class ProductCategoriesService {
  ...
  async findById(id: number, organizationId: string) {
    return this.db
      ...
      .from(productCategories)
      .where(
        and(eq(productCategories.id, id), eq(productCategories.organizationId, organizationId)),
      )
      .then((rows) => rows[0]);
  }
  ...
  async update(
    ...
    data: Partial<Omit<NewProductCategory, "organizationId">>,
  ) {
