...
import { membershipTiers, customers, type NewMembershipTier } from "../db/schema";

export class MembershipTiersService {
    constructor(private db: Database) { }

    async findAll(organizationId: string) {
        return this.db.select().from(membershipTiers)
    ...
    async create(data: Omit<NewMembershipTier, "organizationId">, organizationId: string) {
        const result = await this.db.insert(membershipTiers).values({
            ...data,
            organizationId,
        }).returning();
        return result[0];
    ...
    async update(id: number, organizationId: string, data: Partial<Omit<NewMembershipTier, "organizationId">>) {
        // First verify access
        const existing = await this.findById(id, organizationId);
        if (!existing) return undefined;

        const result = await this.db.update(membershipTiers)
            .set(data)
    ...
    async delete(id: number, organizationId: string) {
        ...
        const result = await this.db.delete(membershipTiers)
            .where(eq(membershipTiers.id, id))
            .returning();
        return result[0];
    }
    ...
    async calculateTierForCustomer(customerId: number, organizationId: string) {
        ...
        const customer = await this.db.select({ totalSpent: customers.totalSpent })
            .from(customers)
