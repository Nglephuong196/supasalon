...
export class CustomerMembershipsService {
    ...
    async findAll(organizationId: string) {
        return this.db.query.customerMemberships.findMany({
            ...
            where: (memberships, { exists, eq, and }) => exists(
                this.db.select().from(customers).where(
                    and(
    ...
    async findByCustomerId(customerId: number, organizationId: string) {
        ...
        const customer = await this.db.select().from(customers)
            .where(and(eq(customers.id, customerId), eq(customers.organizationId, organizationId))).get();

        if (!customer) return [];

        return this.db.select().from(customerMemberships)
    ...
    async create(data: NewCustomerMembership, organizationId: string) {
        // Verify customer belongs to org
        const customer = await this.db.select().from(customers)
            .where(and(eq(customers.id, data.customerId), eq(customers.organizationId, organizationId))).get();
    ...
    async update(id: number, organizationId: string, data: Partial<NewCustomerMembership>) {
        ...
        const result = await this.db.update(customerMemberships).set(data).where(eq(customerMemberships.id, id)).returning();
        return result[0];
    }
    ...
    async delete(id: number, organizationId: string) {
        ...
        const result = await this.db.delete(customerMemberships).where(eq(customerMemberships.id, id)).returning();
        return result[0];
    }
}
