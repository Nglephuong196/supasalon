...
export class ProductsService {
    ...
    async findByCategoryId(categoryId: number, organizationId: string) {
        const category = await this.db.select().from(productCategories)
            .where(and(eq(productCategories.id, categoryId), eq(productCategories.organizationId, organizationId))).get();
        if (!category) return [];
    ...
    async create(data: NewProduct, organizationId: string) {
        const category = await this.db.select().from(productCategories)
            .where(and(eq(productCategories.id, data.categoryId), eq(productCategories.organizationId, organizationId))).get();
