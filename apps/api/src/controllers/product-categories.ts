...
productCategoriesController.get("/", requirePermission(RESOURCES.PRODUCT, ACTIONS.READ), async (c) => {
...
productCategoriesController.get("/:id", requirePermission(RESOURCES.PRODUCT, ACTIONS.READ), async (c) => {
    const service = new ProductCategoriesService(c.get("db"));
    const organization = c.get("organization");
...
productCategoriesController.post("/", requirePermission(RESOURCES.PRODUCT, ACTIONS.CREATE), async (c) => {
    const service = new ProductCategoriesService(c.get("db"));
    const organization = c.get("organization");
...
productCategoriesController.put("/:id", requirePermission(RESOURCES.PRODUCT, ACTIONS.UPDATE), async (c) => {
    const service = new ProductCategoriesService(c.get("db"));
    const organization = c.get("organization");
    const id = parseInt(c.req.param("id"));
...
productCategoriesController.delete("/:id", requirePermission(RESOURCES.PRODUCT, ACTIONS.DELETE), async (c) => {
    const service = new ProductCategoriesService(c.get("db"));
    const organization = c.get("organization");
    const id = parseInt(c.req.param("id"));
