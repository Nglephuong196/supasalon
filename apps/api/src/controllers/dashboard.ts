...
export const dashboardController = new Hono<{ Bindings: Bindings; Variables: Variables }>();
...
function buildChart(range: RangeKey, invoices: any[], from: Date) {
  const unit = "₫";
  if (range === "today") {
...
dashboardController.get("/", requirePermission(RESOURCES.BOOKING, ACTIONS.READ), async (c) => {
  const organization = c.get("organization");
  const range = clampRange(c.req.query("range"));
  const { from, to, prevFrom, prevTo, label, compare } = getRange(range);

  const bookingsService = new BookingsService(c.get("db"));
  const invoicesService = new InvoicesService(c.get("db"));
  const customersService = new CustomersService(c.get("db"));
  const productsService = new ProductsService(c.get("db"));
  const membersService = new MembersService(c.get("db"));
  const servicesService = new ServicesService(c.get("db"));
  ...
  const avgInvoicePrev =
    paidInvoicesPrev.length > 0 ? Math.round(revenuePrev / paidInvoicesPrev.length) : 0;
  ...
  invoices.forEach((invoice: any) => {
    ...
    (invoice.items || []).forEach((item: any) => {
      (item.staffCommissions || []).forEach((staffItem: any) => {
        ...
        const name = membersById.get(staffId) || "Nhân viên";
