...
// Update operation
invoicesController.post("/:id/close", requirePermission(RESOURCES.INVOICE, ACTIONS.UPDATE), async (c) => {
  const service = new InvoicesService(c.get("db"));
  const organization = c.get("organization");
  const id = parseInt(c.req.param("id"));

  const invoice = await service.update(id, organization.id, { isOpenInTab: false });
  if (!invoice) return c.json({ error: "Invoice not found" }, 404);
  return c.json(invoice);
});
