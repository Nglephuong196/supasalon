<script lang="ts">
    ...
    function hasPermission(
        ...
        if (!member || !member.permissions || member.permissions.length === 0)
            return false;
...
<div class="flex flex-col gap-6">
    ...
    <div>
        ...
        <p class="text-muted-foreground mt-1">
            Quản lý cấu hình hệ thống của bạn
        </p>
    </div>
    ...
    <div class="w-full">
        ...
        {#if activeTab === "tiers"}
            <div class="mt-6">
                ...
                <div
                    class="bg-white rounded-xl border border-border/60 p-6 mb-6"
                >
                    <h3
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <span class="text-sm font-medium"
                                >Xét hạng theo điểm tích lũy</span
                            >
                        </label>
                    </div>
                </div>
                ...
                <!-- Tiers Table -->
                <div
                    class="bg-white rounded-xl border border-border/60 overflow-hidden"
                >
                    <div
                        class="flex items-center justify-between p-4 border-b border-border/60"
                    >
                        <h3
                        ...
                        <TableHeader class="bg-gray-50/50">
                            <TableRow class="hover:bg-transparent">
                                ...
                                <TableHead class="font-semibold"
                                    >Chi tiêu tối thiểu</TableHead
                                >
                                <TableHead class="font-semibold"
                                    >Giảm giá mặc định</TableHead
                                >
                                <TableHead class="font-semibold"
                                    >Chi tiêu tối thiểu để giữ hạng</TableHead
                                >
                                <TableHead
                                    class="text-right font-semibold w-[100px]"
                                ></TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {#each data.tiers as tier (tier.id)}
                                ...
                                >
                                    ...
                                    <TableCell>
                                        <span class="text-primary font-medium"
                                            >{tier.discountPercent} %</span
                                        >
                                    </TableCell>
                                    <TableCell>
                                        {#if tier.minSpendingToMaintain}
                                            {formatCurrency(
                                                tier.minSpendingToMaintain,
                                            )}
                                        {:else}
                            ...
                            {#if data.tiers.length === 0}
                                <TableRow>
                                    ...
                                    >
                                        Chưa có hạng khách hàng nào. Hãy tạo
                                        hạng đầu tiên!
                                    </TableCell>
                                </TableRow>
                            {/if}
                    ...
                    >
                        <div class="text-sm text-muted-foreground">
                            Hiển thị từ <span
                                class="font-medium text-foreground">1</span
                            >
                            đến
                            <span class="font-medium text-foreground"
                                >{data.tiers.length}</span
                            >
                            trên tổng số
                            <span class="font-medium text-foreground"
                                >{data.tiers.length}</span
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div
                                class="flex items-center border border-border rounded px-2 py-1"
                            >
            ...
            <div class="mt-6">
                <div class="bg-white rounded-xl border border-border/60 p-6">
                    <p class="text-muted-foreground">
                        Các cài đặt chung sẽ sớm được thêm vào...
                    </p>
                </div>
            </div>
        {/if}
                            ...
                            {#each data.members as member (member.id)}
                                <TableRow>
                                    ...
                                    <TableCell>
                                        ...
                                        >
                                            {getTotalPermissions(member)} quyền được
                                            cấp
                                        </span>
                                    </TableCell>
                                    <TableCell class="text-right">
        ...
        <div class="grid gap-4 py-4">
            <div class="grid gap-2">
                ...
                <Input
                    ...
                    placeholder="Ví dụ: Thương, Thân, Quen..."
                />
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div class="grid gap-2">
                    ...
                    <Input
                        id="minSpending"
                        type="number"
                        bind:value={formData.minSpending}
                    />
                </div>
                <div class="grid gap-2">
            ...
            <div class="grid gap-2">
                <Label for="minSpendingToMaintain"
                    >Chi tiêu để giữ hạng (tùy chọn)</Label
                >
                <Input
        ...
        >
            ...
            <Dialog.Footer>
                <Button
                    variant="outline"
                    type="button"
                    onclick={() => (isPermissionDialogOpen = false)}>Hủy</Button
                >
    ...
    <AlertDialog.Content>
        <AlertDialog.Header>
            ...
            <AlertDialog.Description>
                Hành động này không thể hoàn tác. Hạng khách hàng này sẽ bị xóa
