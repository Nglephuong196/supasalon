...
export const load: PageServerLoad = async ({ fetch, cookies, parent }) => {
    ...
    try {
        ...
        const members: MemberRow[] = membersRes.ok
            ? await membersRes.json()
            : [];
        const services: ServiceRow[] = servicesRes.ok
            ? await servicesRes.json()
            : [];
        const products: ProductRow[] = productsRes.ok
        ...
        const productCategories: CategoryRow[] = productCategoriesRes.ok
            ...
            : [];
        const rules: CommissionRuleRow[] = rulesRes.ok
            ? await rulesRes.json()
            : [];
