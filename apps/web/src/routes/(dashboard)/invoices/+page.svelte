<script lang="ts">
    ...
    const initialDrafts = untrack(() =>
        (data.openInvoices || []).map((inv: any, i: number) =>
            mapInvoiceToDraft(inv, i + 1),
        ),
    );
    let drafts =
        $state<{ id: string; idx: number; data: any }[]>(initialDrafts);
    ...
    $effect(() => {
        const serverOpenIds = new Set(
            (data.openInvoices || []).map((i: any) => i.id.toString()),
        );
        ...
        untrack(() => {
            ...
            drafts = drafts.filter(
                (d) => d.id.startsWith("new-") || serverOpenIds.has(d.id),
            );
...
<div
    class="h-[calc(100vh-6rem)] -m-6 flex flex-col md:flex-row overflow-hidden bg-muted/10"
>
    <!-- MAIN: TABS & WORKSPACE -->
    <div class="flex-1 flex flex-col h-full min-w-0 bg-muted/5">
        <div
            class="bg-background px-4 pt-2 flex items-center justify-between gap-4"
        >
            <!-- Tabs List -->
            <div class="flex-1 overflow-x-auto">
                ...
                >
                    <Tabs.List
                        class="w-max justify-start rounded-none border-b-0 p-0 bg-transparent h-12"
                    >
                        <Tabs.Trigger
                            value="history"
                        ...
                        {#each drafts as draft}
                            {@const customer = data.customers.find(
                                (c: any) =>
                                    c.id.toString() ===
                                    draft.data.customerId?.toString(),
                            )}
                            <Tabs.Trigger
                                value={draft.id}
                                ...
                                <form
                                    ...
                                    use:enhance={() => {
                                        return async ({ result, update }) => {
                                            if (result.type === "success") {
                                                toast.success(
                                                    `Đã hoàn tất các hóa đơn được chọn`,
                                                );
                                                selectedIds = new Set();
                                                await update();
                                            }
