    ...
    try {
      ...
      if (!res.ok) {
        ...
        return fail(res.status, {
          message: err.error || "Không thể tạo lịch hẹn",
        });
      }
      return { success: true };
    } catch (e) {
  ...
  update: async ({ request, fetch, cookies }) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        return fail(res.status, {
          message: err.error || "Không thể cập nhật lịch hẹn",
        });
      }
      return { success: true };
    } catch (e) {
  ...
  delete: async ({ request, fetch, cookies }) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        return fail(res.status, {
          message: err.error || "Không thể xóa lịch hẹn",
        });
      }
      return { success: true };
    } catch (e) {
  ...
  createCustomer: async ({ request, fetch, cookies }) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        return fail(res.status, {
          message: err.error || "Không thể tạo khách hàng",
        });
      }
