...
export const actions: Actions = {
    ...
    createProduct: async ({ request, fetch, cookies }) => {
        ...
        if (!name || name.trim() === '') {
            return fail(400, { missingName: true, message: "Vui lòng nhập tên sản phẩm" });
        }
        if (!categoryId || isNaN(categoryId)) {
            return fail(400, { missingCategory: true, message: "Vui lòng chọn danh mục" });
        }
        if (isNaN(price)) {
            return fail(400, { invalidPrice: true, message: "Giá sản phẩm không hợp lệ" });
        }
        ...
        try {
            const response = await fetch(`${PUBLIC_API_URL}/products`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name, categoryId, price, stock, sku: sku || null, description: description || '' })
            });

            if (!response.ok) {
                const res = await response.json();
                return fail(response.status, { message: res.error || "Không thể tạo sản phẩm" });
            }
            return { success: true };
        } catch (e) {
            return fail(500, { message: "Lỗi máy chủ" });
    ...
    updateProduct: async ({ request, fetch, cookies }) => {
        ...
        try {
            ...
            if (!response.ok) {
                const res = await response.json();
                return fail(response.status, { message: res.error || "Không thể cập nhật sản phẩm" });
            }
            return { success: true };
        } catch (e) {
            return fail(500, { message: "Lỗi máy chủ" });
    ...
    deleteProduct: async ({ request, fetch, cookies }) => {
        ...
        try {
            ...
            if (!response.ok) {
                const res = await response.json();
                return fail(response.status, { message: res.error || "Không thể xóa sản phẩm" });
            }
            return { success: true };
        } catch (e) {
            return fail(500, { message: "Lỗi máy chủ" });
