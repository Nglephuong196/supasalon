...
<div class="h-[calc(100vh-6rem)] flex flex-col gap-4">
  ...
  <div class="flex items-center justify-between flex-none">
    <div>
      <h1 class="text-2xl font-bold tracking-tight text-foreground">Quản lý dịch vụ</h1>
      <p class="text-muted-foreground">Quản lý hình thức và giá dịch vụ tại salon</p>
    </div>
        ...
        >
          ...
          <div class="flex items-center gap-2 w-full sm:w-auto flex-1 justify-end">
            <div class="relative max-w-xs w-full sm:w-64">
              <label for="service-search" class="sr-only">Tìm kiếm dịch vụ</label>
              <Search
                class="absolute left-2.5 top-2.5 h-4 w-4 text-muted-foreground"
                aria-hidden="true"
              />
            ...
            <Button onclick={openCreateService} class="btn-gradient h-9 shrink-0">
                  ...
                  <td class="p-4 align-middle text-right">
                    <DropdownMenu.Root>
                      ...
                      <DropdownMenu.Content align="end" class="border-gray-100 shadow-sm">
                        ...
                        <DropdownMenu.Item onclick={() => openEditService(service)}>
                ...
                <tr>
                  <td colspan="6" class="h-32 text-center">
                    ...
                    >
                      ...
                      >
                        <Layers class="h-6 w-6 text-gray-300" aria-hidden="true" />
                      </div>
...
<Dialog.Root bind:open={isServiceDialogOpen}>
  <Dialog.Content class="sm:max-w-[500px]">
    <Dialog.Header>
      <Dialog.Title>{editingService?.id ? "Sửa dịch vụ" : "Tạo mới dịch vụ"}</Dialog.Title>
    </Dialog.Header>
    <form
      ...
      use:enhance={(e) => {
        ...
        const duration = (form.querySelector('input[name="duration"]') as HTMLInputElement)?.value;
    ...
    >
      ...
      <div class="grid gap-4 py-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="grid gap-2">
            ...
            />
            {#if svcPriceError}
              <span class="text-red-500 text-xs">{svcPriceError}</span>
            {/if}
          </div>
          <div class="grid gap-2">
            ...
            />
            {#if svcDurationError}
              <span class="text-red-500 text-xs">{svcDurationError}</span>
            {/if}
          </div>
        </div>
        <div class="grid gap-2">
          ...
          <div class="relative">
            <Select.Root
              ...
              onValueChange={(v) => {
                svcCategoryError = "";
                if (editingService) editingService.categoryId = parseInt(v);
              }}
            >
              <Select.Trigger class="w-full">
                {data.categories.find(
                  (c: any) => c.id.toString() === editingService?.categoryId?.toString(),
                )?.name || "Chọn danh mục"}
              </Select.Trigger>
              <Select.Content>
          ...
          {#if svcCategoryError}
            <span class="text-red-500 text-xs">{svcCategoryError}</span>
          {/if}
        </div>
        <div class="grid gap-2">
      ...
      <Dialog.Footer>
        <Button variant="outline" type="button" onclick={() => (isServiceDialogOpen = false)}
          >Hủy</Button
        >
...
<AlertDialog.Root bind:open={isDeleteDialogOpen}>
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        ...
        {#if deleteType === "category"}
          Cảnh báo: Xóa danh mục có thể ảnh hưởng đến các dịch vụ trong danh mục đó.
        {:else}
    ...
    <form
      action={deleteType === "category" ? "?/deleteCategory" : "?/deleteService"}
      method="POST"
