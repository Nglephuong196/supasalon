...
import { customerKeys, type PaginatedResponse } from "@/lib/queries/customers"
...
export function CustomersPageClient({ initialData }: CustomersPageClientProps) {
  ...
  // Hydrate cache with server data on first render
  // TanStack Query will use initialData and refetch in background if stale
  const { data: paginatedData, isLoading, error } = useCustomersPaginated(page, PAGE_SIZE, debouncedSearch)
  
  // Use initialData for first page with no search, otherwise use query data
  const effectiveData = (page === 1 && !debouncedSearch && !paginatedData) 
    ? initialData 
  ...
  const customers = effectiveData.data
  const total = effectiveData.total
  const totalPages = effectiveData.totalPages
