...
export function formatPermissions(permissions: Record<string, string[]>): Permissions {
    ...
    for (const [resource, actions] of Object.entries(permissions)) {
        ...
        if (!validResources.includes(resource as Resource)) {
            throw new Error(`Invalid resource: ${resource}. Valid resources are: ${validResources.join(', ')}`);
        }
        ...
        for (const action of actions) {
            if (!validActions.includes(action as Action)) {
                throw new Error(`Invalid action: ${action} for resource ${resource}. Valid actions are: ${validActions.join(', ')}`);
            }
        }
